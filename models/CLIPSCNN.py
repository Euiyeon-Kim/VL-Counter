...
class CLIPSMatMulCNNv1(BaseModel):
    """
        Use ViT feature
    """
    def __init__(self, args):
        super(CLIPSMatMulCNNv1, self).__init__(args=args)
    ...
    def predict(self, imgs, class_names, set_img_dict=False):
        ...
        _, features = self.img_backbone(imgs)      # [B, 1, 512], [B, 1024, 512]

        similarity = torch.matmul(features, txt_embeddings.unsqueeze(-1)).reshape(B, npH, npW, 1).permute(0, 3, 1, 2)
...
class CLIPSCorrCNNv1(BaseModel):
    ...
    def predict(self, imgs, class_names, set_img_dict=False):
        ...
        B, _, H, W = imgs.size()
        npH, npW = H // self.args.patch_size, W // self.args.patch_size
        _, surg_features, features = self.img_backbone(imgs)      # [B, 1, 512], [B, 1024, 512]
